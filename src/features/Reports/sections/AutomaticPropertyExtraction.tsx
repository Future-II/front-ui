...
const AutomaticPropertyExtraction: React.FC = () => {
  ...
  const { reports, loading, error, fetchReports, searchReports } =
    useReportsData();
    ...
    <>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div className="grid grid-cols-3 gap-4 mb-6">
          <input
            type="text"
            placeholder={t("automation.automation report.report name") || "Report name"}
            value={reportName}
          ...
          <input
            type="text"
            placeholder={t("automation.automation report.site") || "Site"}
            value={siteLocation}
          ...
          <select
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          >
            <option value="">{t("automation.automation report.condition.a") || "Select condition"}</option>
            <option value="مكتمل">{t("automation.automation report.condition.b") || "Completed"}</option>
            <option value="معلق">{t("automation.automation report.condition.c") || "Pending"}</option>
            <option value="مسودة">{t("automation.automation report.condition.d") || "Draft"}</option>
          </select>
          <input
            type="text"
            placeholder={t("automation.automation report.reference") || "Reference number"}
          ...
          <input
            type="text"
            placeholder={t("automation.automation report.property") || "Property type"}
            value={propertyType}
        ...
        <div className="flex gap-3">
          <button
            ...
            className="bg-blue-500 text-white px-6 py-2 rounded-md"
          >
            {loading ? "Searching..." : t("automation.automation report.button") || "Search"}
          </button>
          <button
            type="button"
            onClick={clearSearch}
